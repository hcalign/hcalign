...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, video_frame=-1, start=0, end=-1):
        if end == -1: end = self.layers
        if start == 0 and end == self.layers:
...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, video_frame=-1, start=0, end=-1):
        ...
        x = self.transformer(x, video_frame=video_frame, start=start, end=end)
    ...
    def encode_image(self, image: torch.Tensor, video_frame=-1, return_hidden=False, start=0, end=-1, ):
        if end == -1: end = self.n_layers
        hidden = self.forward(image.type(self.dtype), video_frame=video_frame, start=start, end=end)
        if end != self.n_layers:
            return hidden
      
        hidden = self.ln_post(hidden) @ self.proj
        x = hidden[:, 0, :]
        if return_hidden:
            return x, hidden
        return x
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, image_diff=None, return_hidden=False, video_frame=-1, start=0, end=-1):
        """
            split visual backbone 
        """
        return self.visual.encode_image(image,  video_frame=video_frame, return_hidden=return_hidden,start=start, end=end)
